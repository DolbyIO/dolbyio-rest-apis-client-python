...
from dolbyio_rest_apis.media import transcode
import json
...
def add_arguments(sub_parsers: _SubParsersAction) -> None:
    parser = sub_parsers.add_parser(command_name(), help='Transcode API')
    ...
    start_parser = sub_parsers_io.add_parser(
        'start',
        help='Start transcoding to modify the resolutions, bitrates, and formats for your media.'
    )
    ...
    result_parser = sub_parsers_io.add_parser('result', help='Gets Transcode Results')
...
async def execute_command(args: Namespace) -> None:
    ...
    if args.sub_command == 'start':
        ...
        job_id = args.job_id
        job_result = await transcode.get_results(
            access_token=access_token,
            job_id=job_id
        )
